...
1. (`user@hostname>`) Add a new entry for the hostname `kubernetes` to the
   external system's `/etc/hosts` file.

   The `kubernetes` hostname should correspond to the CAN IP address on `ncn-m001`.
   For example, you might use the following command on a system with CSM 1.2.
